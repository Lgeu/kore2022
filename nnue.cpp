...
[[maybe_unused]] static void TestPrediction(const string kif_filename,
    ...
    while (true) {
        ...
        if (                                          /*state.step_ < 100 || */
            ...
            agent_mcts.nnue.Forward(batch_size, shipyard_feature_tensor,
                                    global_feature_tensor, value_tensor,
                                    type_tensor, code_tensor);
            agent_mcts.spawn_decoder.Forward(batch_size, code_tensor,
                                             spawn_n_ships_tensor);
            agent_mcts.move_decoder.Forward(
                batch_size, code_tensor, move_n_ships_tensor,
                move_relative_position_tensor, move_n_steps_tensor);
            agent_mcts.attack_decoder.Forward(
                batch_size, code_tensor, attack_n_ships_tensor,
                attack_relative_position_tensor, attack_direction_tensor);
            agent_mcts.convert_decoder.Forward(
